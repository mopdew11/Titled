...
if(inventory = false)
{
	...
	if(primary != -1)
		draw_sprite_ext(spr_items,primary,sX + spriteAdd,8 + spriteAdd,2,2,0,c_white,1);
	
	//SECONDARY WEAPON
	sX = display_get_gui_width() - 72;
	draw_sprite_ext(spr_inventorySlot,0,sX,8,2,2,0,c_white,1);	
	if(secondary != -1)
		draw_sprite_ext(spr_items,secondary,sX + spriteAdd,8 + spriteAdd,2,2,0,c_white,1);
	
	//MELEE WEAPON
	draw_sprite_ext(spr_inventorySlot,1,sX,80,2,2,0,c_white,1);	
	if(melee != -1)
		draw_sprite_ext(spr_items,melee,sX + spriteAdd,80 + spriteAdd,2,2,0,c_white,1);
